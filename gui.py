...
#-------------------------------------------------#
    ...
    def update_score(self):
        self.score_label.config(text= "score: {:02d}".format(self.__boggle.get_score()))
    #run methos(incomplete)
    def run(self):
        self.end_time = time.time() + self.__duration
        self.update_timer()
    ...
    def create_buttons_in_lower_frame(self) -> None:
        ...
        for row in range(ROWS):
            ...
            for col in range(COLS):
                ...
                button = tk.Button(self.button_grid_frame, text=self.__boggle.get_value_by_location(row,col).upper(), **BUTTON_STYLE,command= lambda r=row,c=col: self._display_label.config(text=self.__boggle.get_value_by_location(r,c)))
